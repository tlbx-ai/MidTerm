...
- `src/Ai.Tlbx.MidTerm/openapi/openapi.json` is generated by `Ai.Tlbx.MidTerm.OpenApi`
- `src/Ai.Tlbx.MidTerm/src/ts/api.generated.ts` is generated from that OpenAPI document during `frontend-build.ps1`
- If the generated contract is wrong, fix the source in `src/Ai.Tlbx.MidTerm.Api/` and `src/Ai.Tlbx.MidTerm.OpenApi/`, then rebuild
- When adding or changing REST endpoints, update the API contract layer first:
  1. Add/update endpoint definitions under `src/Ai.Tlbx.MidTerm.Api/Endpoints/`
  2. Add/update handler interfaces under `src/Ai.Tlbx.MidTerm.Api/Handlers/`
  3. Add/update stub handlers under `src/Ai.Tlbx.MidTerm.OpenApi/Stubs/`
  4. Regenerate the OpenAPI document and frontend types by running the normal build
- Never “patch the output” in `openapi.json` or `api.generated.ts` to make the frontend compile
