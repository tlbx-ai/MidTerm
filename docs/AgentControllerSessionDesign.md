...
- Codex model choices must come from the runtime's paginated `model/list` catalog in provider order. tlbx must preserve the currently active custom/default model, follow every `nextCursor`, and must not present a hardcoded Codex model catalog when discovery is unavailable.
