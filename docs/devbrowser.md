...
Agents can explicitly recover from stale preview ownership with `mt_claim_preview` or `mt_open --claim <url>`. Normal `mt_open <url>` reclaims a stale owner to the attached leading browser but preserves the user's active tlbx session. If the caller is in the foreground, its preview opens in the dock; if it is a background session, tlbx attaches a hidden automation frame instead. `mt_open --activate <url>` is the explicit opt-in for selecting the caller's session and showing its dock. The claim path assigns the selected `(sessionId, previewName)` to the connected leading tlbx browser UI listener, then normal `open`, `reload`, and browser-command routing use that owner. `mt_status` includes a compact `bridge phase` field such as `no-ui-client`, `no-target`, `owner-offline`, `preview-frame-disconnected`, `ambiguous-preview`, or `ready`, plus a one-line recovery hint.

`mt_open` and `/api/browser/open` require the selected browser bridge to reconnect before they report success. Explicitly activated opens require a visible preview frame; background opens accept the scoped hidden automation frame so they can remain controllable without changing the user's terminal or dock. The parent posts `mt-refresh-browser-state` into a frame whenever its visibility changes so the injected bridge refreshes its visibility/focus flags and reconnects when needed.
