...
public sealed class SessionAppServerControlHostRuntimeServiceTests
{
    ...
    [Fact]
    public async Task SessionAppServerControlRuntimeService_CanDelegateToMtAgentHostClaudeMode()
    {
        using var fakeClaude = FakeClaudePathScope.Create();
        var settingsRoot = Path.Combine(Path.GetTempPath(), "midterm-appServerControl-claude-settings-" + Guid.NewGuid().ToString("N"));
        ...
        try
        {
            await using var hostRuntime = new SessionAppServerControlHostRuntimeService(settingsService, mode: "codex");
            await using var sessionManager = new TtyHostSessionManager();
            var profileService = new AiCliProfileService();
            await using var runtime = new SessionAppServerControlRuntimeService(sessionManager, profileService, hostRuntime);
