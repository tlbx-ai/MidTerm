...
public sealed class TlbxCliScriptWriterTests : IDisposable
{
    ...
    public void WriteScripts_WritesSessionScopedPreviewHelpers()
    {
        ...
        Assert.Contains("local claim=0 activate=0 url=\"\" open_out status", shell, StringComparison.Ordinal);
        Assert.Contains("--claim) claim=1", shell, StringComparison.Ordinal);
        Assert.Contains("--activate) activate=1", shell, StringComparison.Ordinal);
        ...
        Assert.Contains("\\\"activateSession\\\":$activate_json", shell, StringComparison.Ordinal);
        ...
        Assert.Contains("param([string]$Url, [switch]$Claim, [switch]$Activate)", powershell, StringComparison.Ordinal);
        ...
        Assert.Contains("activateSession=[bool]$Activate", powershell, StringComparison.Ordinal);
    ...
    public void Ensure_WritesAgentsGuidanceWithSessionScopedPreviewWorkflow()
    {
        ...
        Assert.Contains("`mt_open` sets the target and waits until that preview is controllable", agents, StringComparison.Ordinal);
        Assert.Contains("a background terminal gets a hidden automation frame and does not steal focus", agents, StringComparison.Ordinal);
        Assert.Contains("Use `mt_open --activate <url>` only when a visible session switch is intentional", agents, StringComparison.Ordinal);
