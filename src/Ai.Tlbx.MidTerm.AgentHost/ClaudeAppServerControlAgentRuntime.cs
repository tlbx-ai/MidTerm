...
internal sealed class ClaudeAppServerControlAgentRuntime : IAppServerControlAgentRuntime
{
    ...
    public async Task<HostCommandOutcome> ExecuteAsync(AppServerControlHostCommandEnvelope command, CancellationToken ct)
    {
        ...
        try
        {
            return command.Type switch
            {
                ...
                "request.resolve" => throw new InvalidOperationException("Claude App Server Controller approval resolution is not supported by the current Claude runtime integration."),
                "user-input.resolve" => throw new InvalidOperationException("Claude App Server Controller interview/user-input resolution is not supported by the current Claude runtime integration."),
    ...
    private async Task FinalizeExitAsync(Process process)
    {
        ...
        {
            _emit(CreateEvent("turn.completed", _activeTurnId, null, null, "claude.stream-json", "process.exit", new { exitCode = process.ExitCode }, appServerControlEvent =>
            {
                appServerControlEvent.TurnCompleted = new AppServerControlProviderTurnCompletedPayload
                {
                    State = "failed",
    ...
    private void HandleStreamEvent(JsonElement root, string rawLine)
    {
        ...
        switch (eventType)
        {
            case "message_start":
                ...
                {
                    _emit(CreateEvent("session.state.changed", _activeTurnId, null, null, "claude.stream-json", "message_start", root, appServerControlEvent =>
                    {
                        appServerControlEvent.SessionState = new AppServerControlProviderSessionStatePayload
                        {
                            State = "running",
                    ...
                    {
                        _turnStarted = true;
                        _emit(CreateEvent("turn.started", _activeTurnId, null, null, "claude.stream-json", "message_start", root, appServerControlEvent =>
                        {
    ...
    private void HandleContentBlockStart(JsonElement root, string rawLine)
    {
        ...
        _emit(CreateEvent("item.started", _activeTurnId, itemId, null, "claude.stream-json", "content_block_start", root, appServerControlEvent =>
        {
    ...
    private void HandleContentBlockDelta(JsonElement root, string rawLine)
    {
        ...
        switch (deltaType)
        {
            case "text_delta":
            {
                ...
                {
                    _emit(CreateEvent("content.delta", _activeTurnId, state.ItemId, null, "claude.stream-json", "content_block_delta", root, appServerControlEvent =>
                    {
            ...
            case "input_json_delta":
            {
                ...
                if (_tools.TryGetValue(toolKey, out var tool))
                {
                    ...
                    _emit(CreateEvent("item.updated", _activeTurnId, tool.ItemId, null, "claude.stream-json", "content_block_delta", root, appServerControlEvent =>
                    {
                        appServerControlEvent.Item = new AppServerControlProviderItemPayload
                        {
    ...
    private void EmitAssistantMessage(string text, JsonElement root, string rawLine)
    {
        ...
        _emit(CreateEvent("item.completed", _activeTurnId, itemId, null, "claude.stream-json", "assistant", root, appServerControlEvent =>
        {
            appServerControlEvent.Item = new AppServerControlProviderItemPayload
            {
    ...
    private void HandleUser(JsonElement root, string rawLine)
    {
        ...
        while (toolResultItems.MoveNext())
        {
            ...
            {
                _emit(CreateEvent("content.delta", _activeTurnId, tool.ItemId, null, "claude.stream-json", "tool_result", root, appServerControlEvent =>
                {
                    appServerControlEvent.ContentDelta = new AppServerControlProviderContentDeltaPayload
                    {
            ...
            _emit(CreateEvent("item.completed", _activeTurnId, tool.ItemId, null, "claude.stream-json", "tool_result", root, appServerControlEvent =>
            {
                appServerControlEvent.Item = new AppServerControlProviderItemPayload
                {
                    ItemType = tool.ItemType,
    ...
    private void HandleResult(JsonElement root, string rawLine)
    {
        ...
        _emit(CreateEvent("turn.completed", _activeTurnId, null, null, "claude.stream-json", "result", root, appServerControlEvent =>
        {
            appServerControlEvent.TurnCompleted = new AppServerControlProviderTurnCompletedPayload
            {
        ...
        _emit(CreateEvent("session.state.changed", _activeTurnId, null, null, "claude.stream-json", "result", root, appServerControlEvent =>
        {
            appServerControlEvent.SessionState = new AppServerControlProviderSessionStatePayload
            {
    ...
    private void EnsureProviderThreadId(JsonElement root)
    {
        ...
        _emit(CreateEvent("thread.started", null, null, null, "claude.stream-json", "session_id", root, appServerControlEvent =>
        {
            appServerControlEvent.ThreadState = new AppServerControlProviderThreadStatePayload
            {
                State = "active",
    ...
    private void EmitAssistantDelta(string text, JsonElement root, string rawLine)
    {
        ...
        var itemId = $"assistant:{_activeTurnId}";
        _emit(CreateEvent("content.delta", _activeTurnId, itemId, null, "claude.stream-json", "assistant", root, appServerControlEvent =>
        {
            appServerControlEvent.ContentDelta = new AppServerControlProviderContentDeltaPayload
            {
