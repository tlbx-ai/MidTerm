...
const LOG_LEVEL_MAP: Record<LogSeverity, LogLevel> = {
...
export function applyReceivedSettings(settings: MidTermSettingsPublic): void {
  ...
  const logLevelValue = settings.logLevel ?? 'warn';
  const logLevel = LOG_LEVEL_MAP[logLevelValue] ?? LogLevel.Warn;
...
export function saveAllSettings(): void {
  ...
  const settings: MidTermSettingsUpdate = {
    ...
    logLevel: getElementValue('setting-log-level', 'warn') as LogSeverity,
  };
