...
describe('appServerControlAttachmentSubmission', () => {
  it('uploads all queued composer attachments before queuing the AppServerControl turn', async () => {
    const uploadFile = vi
    ...
    const { submitAppServerControlComposerDraft } =
      await import('./appServerControlAttachmentSubmission');
    const result = await submitAppServerControlComposerDraft({
      sessionId: 's1',
      draft: {
        nextOrdinalByKind: {},
  ...
  it('fails fast when any upload does not produce a path', async () => {
    ...
    await expect(
      submitAppServerControlComposerDraft({
        sessionId: 's1',
        draft: {
          nextOrdinalByKind: {},
          parts: [{ kind: 'text', text: 'Inspect this image.' }],
